...
const createWindow = () => {
  ...
  // Reacting to commands sent from the Vue.js UI
  ipcMain.on("set-config", (_, data) => writeConfig(data));
  ipcMain.on("get-config", () =>
    mainWindow.webContents.send("config", loadConfig())
  );

  // TODO: to make it easier to understand, use a if statement on mode
  initWatcher();
  initCron();
};
